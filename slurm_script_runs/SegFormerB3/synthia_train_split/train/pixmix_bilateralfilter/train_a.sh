...
#SBATCH --job-name=segformer_b3_gta_train-split_pixmix_bilateralfilter_train_a

## Name of the log file
#SBATCH --output=train_a-%j.out

max_iters=40000

## The config that is used for training and testing
main_config="./local_configs/segformer/B3/segformer.b3.512x512.gta2cs.40k.batch2_pixmix_bilateralfilter.py"

## The name of the working dir where the results are saved. at the end the prefix "_[a..z]" is added.
work_dir_prefix="./work_dirs/synthia_train_split/segformer_b3/segformer.b3.512x512.gta2cs.40k.batch2_pixmix_bilateralfilter"
...
## Defines the start seed. The first model is trained wth this value. After this, the seed is increased by one for the next model
start_seed=0

## Defines on which dataset the models are trained
train_dataset="synthia_train"
#train_dataset="gta_full"
#train_dataset="synthia_train"
#train_dataset="synthia_full"
...
if [ "$train_dataset" = "gta_full" ]; then
    ...
    echo -e "Train Dataset:\n   Synthia"
elif [ "$train_dataset" = "cs_full" ] ; then
    data_train_type="CityscapesDataset"
    data_train_data_root="data/cityscapes/"
    data_train_img_dir="leftImg8bit/val"
    data_train_ann_dir="gtFine/val"
else ## "synthia_full"
    data_train_type="SynthiaDataset"
