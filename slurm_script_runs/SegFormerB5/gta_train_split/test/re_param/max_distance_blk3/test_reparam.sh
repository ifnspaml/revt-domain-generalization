...
#SBATCH --job-name=Segformerb5

## Name of the log file
#SBATCH --output=test_<a|b|c>-%j.out

## The config that is used for training and testing
main_config="./local_configs/segformer/B5/cs/segformer.b5.512x512.gta2cs.40k.batch2_max_distance.py"

## The name of the working dir where the results are saved. at the end the prefix "_[a..z]" is added.
declare -a work_dirs=("./work_dirs/gta_train/segformer_b5/segformer.b5.512x512.gta2cs.40k.batch2_max_distance_blk3_a/latest.pth"
                      "./work_dirs/gta_train/segformer_b5/segformer.b5.512x512.gta2cs.40k.batch2_max_distance_blk3_b/latest.pth"
                      "./work_dirs/gta_train/segformer_b5/segformer.b5.512x512.gta2cs.40k.batch2_max_distance_blk3_c/latest.pth")
...
for regex_idx in "${!regexes[@]}"; do
  ...
  for i in "${!merge_orders[@]}"; do
    ...
    srun python tools/model_reparametrization.py\
     ...
     --weights-filter "${regexes[regex_idx]}" --no-warnings
