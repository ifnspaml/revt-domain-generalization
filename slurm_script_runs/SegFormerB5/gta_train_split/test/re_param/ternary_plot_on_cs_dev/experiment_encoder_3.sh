...
srun python ./tools/test_reparametrization_3d.py\
 ...
 "slurm_script_runs/SegFormerB5/gta_train_split/test/re_param/ternary_plot_on_cs_dev/res_array_decoder_3_cs_val.npy"\
 --launcher="slurm"\
 --gpu-collect\
 --eval-options efficient_test='True'\
 --weights-filter "backbone.*"\
 --options dist_params.backend='nccl' dist_params.port=${port}\
        data.test.type="GTADataset"\
        dataset_type="GTADataset"\
        data_root="data/"\
        data.test.data_root="data/"\
        data.test.img_dir="gta/images"\
        data.test.ann_dir="gta/labels"\
        data.test.split="gta_splits/val_split.txt"\
 --resume-from "slurm_script_runs/SegFormerB5/gta_train_split/test/re_param/ternary_plot_on_cs_dev/res_array_decoder_3_cs_val.npy"\
 --checkpoints\
