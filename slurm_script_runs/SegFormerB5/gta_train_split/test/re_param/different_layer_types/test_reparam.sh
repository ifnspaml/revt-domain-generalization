...
#only attention_layers, only convolution layers, only mix FFN, only MLP, only path embedding,
declare -a regexes=("backbone.*attn.*" "backbone.*((block.*mlp.*dwconv)|patch_embed).*" "backbone.*block.*mlp.*" "backbone.*block.*mlp.*fc.*" "backbone.patch_embed.*")
