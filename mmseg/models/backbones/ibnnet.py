## This code is copied from https://github.com/XingangPan/IBN-Net/blob/master/ibnnet/resnet_ibn.py, https://github.com/XingangPan/IBN-Net/blob/master/ibnnet/modules.py
...
class Bottleneck_IBN(nn.Module):
    ...
    def __init__(self, inplanes, planes, ibn=None, stride=1, downsample=None, affine=True):
        ...
        self.IN = nn.InstanceNorm2d(planes * 4, affine=affine) if ibn == 'b' else None
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class ResNet_IBN(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
        if ibn_cfg[0] == 'b':
            self.bn1 = nn.InstanceNorm2d(64, affine=affine)
        else:
            self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)
        self.affine = affine
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, 64, layers[0], stride=strides[0], ibn=ibn_cfg[0])
        self.layer2 = self._make_layer(block, 128, layers[1], stride=strides[1], ibn=ibn_cfg[1])
        self.layer3 = self._make_layer(block, 256, layers[2], stride=strides[2], ibn=ibn_cfg[2])
        self.layer4 = self._make_layer(block, 512, layers[3], stride=strides[3], ibn=ibn_cfg[3])
    ...
    def _make_layer(self, block, planes, blocks, stride=1, ibn=None):
        ...
        layers.append(block(self.inplanes, planes,
                            ...
                            stride, downsample, affine=self.affine))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes,
                                None if (ibn == 'b' and i < blocks-1) else ibn, affine=self.affine))
