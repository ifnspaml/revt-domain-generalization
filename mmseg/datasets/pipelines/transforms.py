...
class PhotoMetricDistortion(object):
    ...
    def contrast(self, img):
        ...
        if random.randint(2):
            ...
            return self.convert(
                img,
                alpha=random.uniform(self.contrast_lower, self.contrast_upper)), changed
        return img, changed
    ...
    def __call__(self, results):
        ...
        img = results['img']
        # random brightness
        img, results['PhotoMetricDistortionBrightness'] = self.brightness(img)

        # mode == 0 --> do random contrast first
        # mode == 1 --> do random contrast last
        mode = random.randint(2)
        if mode == 1:
            img, results['PhotoMetricDistortionContrast'] = self.contrast(img)

        # random saturation
        img, results['PhotoMetricDistortionSaturation'] = self.saturation(img)

        # random hue
        img, results['PhotoMetricDistortionHue'] = self.hue(img)
